...
REPO_DIR = "/app/secrets_scan_dir"
REPORT_FILE = "{}_secrets-scan-report_{}.csv"
GIT_REV_LIST_STDOUT = ""
OUTPUT_DIR = "/app/reports"  # Directory to save the CSV report
...
def clone_repo(repo_url):
    ...
    result = subprocess.run(["git", "clone", repo_url, REPO_DIR], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def talisman():
    ...
    TALISMAN_BINARY_PATH = "/usr/local/bin/talisman"
    # Run talisman
    print("Running Talisman Scan\n")
    result = subprocess.run([TALISMAN_BINARY_PATH, "--scanWithHtml", "--reportDirectory", "talisman_report"])
...
def main():
    ...
    print("*******************************************Warning*****************************************\n\n")
    # Get repo URL from environment variable or command-line argument
